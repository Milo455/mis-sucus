...
Create a `.env` file in the project root with the credentials for your Firebase project. Ensure Firestore is enabled in the Firebase console. If you open the app directly in the browser, also copy `env.template.js` to `env.js`, fill in your credentials and include it before `firebase-init.js` in your HTML. The application will throw an error if these variables are missing, so make sure they are defined.
